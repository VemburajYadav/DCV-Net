...
def readPngFlowKubrik(filename) -> np.ndarray:
    data_ranges_filepath = getConfigPathFromFlowFilepathKubrik(filename)
    rescale_range = readFlowRangeKubrik(data_ranges_filepath)
...
def getConfigPathFromFlowFilepathKubrik(filename):
    scene = filename.split("/")[-2]
    base_dir = filename.split("/")[:-5]
    base_dir = Path("/".join(base_dir))
    config_path = base_dir / "configs_1k" / "configs" / "1k" / scene / "data_ranges.json"
    return str(config_path)

def readFlowRangeKubrik(json_path):
    with open(json_path, 'r') as f:
        data = json.load(f)

    # Safely access forward_flow min and max
    forward = data.get('forward_flow', {})

    min_val = forward.get('min')
    max_val = forward.get('max')

    if min_val is not None and max_val is not None:
        print(f"Forward Flow → Min: {min_val}, Max: {max_val}")
    else:
        print("Missing min/max in forward_flow")
