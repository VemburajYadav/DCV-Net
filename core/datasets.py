...
class FlowDataset(data.Dataset):
    def __init__(self, aug_params=None, sparse=False, return_only_spatial=False):
        self.augmentor = None
        self.sparse = sparse
...
class MpiSintel(FlowDataset):
    def __init__(self, aug_params=None, split='training',
        ...
        # root = '/home/vyadav/mnt/Datasets/mpi_sintel/raw'
...
class FlyingChairs(FlowDataset):
    def __init__(self, aug_params=None, split='train',
        ...
        root_subset = os.path.join(root, split)
        img1_list, img2_list, flow_list = [],  [],  []

        for dp, ds, filenames in os.walk(root_subset):
            img1_list += [os.path.join(dp, fs) for fs in filenames if "img1" in fs]
            img2_list += [os.path.join(dp, fs) for fs in filenames if "img2" in fs]
            flow_list += [os.path.join(dp, fs) for fs in filenames if "flow" in fs]

        img1_list, img2_list, flow_list = sorted(img1_list), sorted(img2_list), sorted(flow_list)

        self.image_list = list(zip(*(img1_list, img2_list)))
    ...
    def __init__(self, aug_params=None, root='/netscratch/vemburaj/datasets/flying_things',
        ...
        for cam in ['left']:
            for direction in ['into_future', 'into_past']:
                image_dirs = sorted(glob(osp.join(root, dstype, basename)))
                image_dirs = sorted([osp.join(f, cam) for f in image_dirs])

                flow_dirs = sorted(glob(osp.join(root, 'optical_flow/' + basename)))
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    if args.stage == 'chairs':
        ...
        train_dataset = FlyingChairs(aug_params, split='train',
                                     return_only_spatial=args.coarse_supervision or args.dynamic_coarse_supervision)
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=args.batch_size, 
                                   pin_memory=False, shuffle=True, worker_init_fn=seed_worker,
                                   num_workers=int(os.environ['SLURM_CPUS_PER_TASK']), drop_last=True)
