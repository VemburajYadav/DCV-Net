...
SAVE_DIR = data_root / "SOTA_Comparison_Videos-Tiled" / SCENE_DIR
...
if __name__ == '__main__':
    ...
    for i in range(max_frames):
        ...
        for scene in SCENES:
            ...
            flow_dir_dcv = SCENE_DIR / scene / (STAGE + TILED) / "flow_pred_with_epe"
            flow_dir_ff = SCENE_DIR / scene / (STAGE + "-flowformer" + TILED) / "flow_pred_with_epe"
            flow_dir_gmflow = SCENE_DIR / scene / (STAGE + "-gmflow" + TILED) / "flow_pred_with_epe"
            ...
            flow_paths_ff = list(sorted(glob(os.path.join(str(flow_dir_ff), "*.png"))))
            flow_paths_gmflow = list(sorted(glob(os.path.join(str(flow_dir_gmflow), "*.png"))))
            ...
            flow_ff = np.array(read_gen(flow_paths_ff[idx])).astype(np.uint8)
            flow_gmflow = np.array(read_gen(flow_paths_gmflow[idx])).astype(np.uint8)
            ...
            data_grid += [[put_text_in_frame(flow_ff, "FlowFormer", location="top-left", bg="dark"),
                           put_text_in_frame(flow_gmflow, "GMFlow", location="top-left", bg="dark")]]
